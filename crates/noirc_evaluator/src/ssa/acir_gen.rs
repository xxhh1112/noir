...
use acvm::acir::circuit::opcodes::{Brillig, Opcode as AcirOpcode, OracleData};
...
use super::function::RuntimeType;
...
pub(crate) fn unsafe_call(
    ...
    for (i, a) in arguments.iter().enumerate() {
        inputs
            .push(var_cache.get_or_compute_internal_var_unwrap(*a, evaluator, ctx).to_expression());
    ...
    let opcode = match &f.kind {
        RuntimeType::Unsafe => {
            let mut code = f.brillig_code.clone();
            // code.push(BrilligOpcode::Bootstrap{register_allocation_indices:register_load});
            AcirOpcode::Brillig(Brillig { inputs, outputs, bytecode: code })
        }
        RuntimeType::Oracle(oracle_name) => AcirOpcode::Oracle(OracleData {
        ...
        RuntimeType::Acvm => {
            panic!("ICA - Acvm functions are safe")
        }
    };

    evaluator.push_opcode(opcode);
