...
export class SingleFft implements Fft {
  ...
  public async fft(coefficients: Uint8Array, constant: Uint8Array) {
    ...
    await this.wasm.transferToHeap(
      Transfer(coefficients, [coefficients.buffer]) as any,
      newPtr
    );
    await this.wasm.transferToHeap(
      Transfer(constant, [constant.buffer]) as any,
      0
    );
    await this.wasm.call(
      "coset_fft_with_generator_shift",
      newPtr,
      0,
      this.domainPtr
    );
    const result = await this.wasm.sliceMemory(
      newPtr,
      newPtr + circuitSize * 32
    );
