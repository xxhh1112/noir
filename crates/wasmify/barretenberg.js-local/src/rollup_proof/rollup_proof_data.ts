...
const parseHeaderInputs = (proofData: Buffer) => {
  ...
  for (let i = 0; i < RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK; ++i) {
    defiDepositSums.push(
      toBigIntBE(proofData.slice(startIndex, startIndex + 32))
    );
    startIndex += 32;
  }
...
export class RollupProofData {
  ...
  static LENGTH_ROLLUP_HEADER_INPUTS =
    RollupProofData.NUM_ROLLUP_HEADER_INPUTS * 32;
  ...
  constructor(
    ...
    if (bridgeIds.length !== RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK) {
      throw new Error(
        `Expect bridgeIds to be an array of size ${RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK}.`
      );
    }
    if (defiDepositSums.length !== RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK) {
      throw new Error(
        `Expect defiDepositSums to be an array of size ${RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK}.`
      );
    }
    if (totalTxFees.length !== RollupProofData.NUMBER_OF_ASSETS) {
      throw new Error(
        `Expect totalTxFees to be an array of size ${RollupProofData.NUMBER_OF_ASSETS}.`
      );
    }
    if (
      defiInteractionNotes.length !== RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK
    ) {
      throw new Error(
  ...
  public get rollupHash(): Buffer {
    if (!this.rollupHash_) {
      ...
      this.rollupHash_ = createHash("sha256")
    ...
    return Array.from({ length: rollupSize })
      .map((_, i) => {
        ...
        return createTxId(
          proofData.slice(
            innerProofStart,
            innerProofStart + InnerProofData.LENGTH
          )
        );
      })
  ...
  static fromBuffer(proofData: Buffer) {
    ...
    for (let i = 0; i < rollupSize; ++i) {
      const innerData = proofData.slice(
        startIndex,
  ...
  static randomData(
    ...
    const ipd =
      innerProofData === undefined
        ? new Array(numTxs)
            .fill(0)
            .map(() =>
              InnerProofData.fromBuffer(Buffer.alloc(InnerProofData.LENGTH))
            )
        : innerProofData;
    ...
    return new RollupProofData(
      ...
      bridgeIds
        ...
        .concat(
          new Array(
            RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK - bridgeIds.length
          )
            .fill(0)
            .map(() => Buffer.alloc(32))
        ),
      ...
      new Array(RollupProofData.NUM_BRIDGE_CALLS_PER_BLOCK)
        .fill(0)
        .map(() => Buffer.alloc(32)),
      Buffer.alloc(32),
      Buffer.alloc(32),
