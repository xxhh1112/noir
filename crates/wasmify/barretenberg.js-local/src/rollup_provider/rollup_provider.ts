...
export const pendingTxToJson = ({
  txId,
  noteCommitment1,
  noteCommitment2,
}: PendingTx): PendingTxJson => ({
  txId: txId.toString(),
  noteCommitment1: noteCommitment1.toString("hex"),
  noteCommitment2: noteCommitment2.toString("hex"),
});

export const pendingTxFromJson = ({
  txId,
  noteCommitment1,
  noteCommitment2,
}: PendingTxJson): PendingTx => ({
...
export const initialWorldStateToBuffer = (
...
export const initialWorldStateFromBuffer = (
  data: Buffer
): InitialWorldState => {
...
export const depositTxToJson = ({
  assetId,
  value,
  publicOwner,
}: DepositTx): DepositTxJson => ({
  assetId,
  value: value.toString(),
  publicOwner: publicOwner.toString(),
});

export const depositTxFromJson = ({
  assetId,
  value,
  publicOwner,
}: DepositTxJson): DepositTx => ({
  assetId,
  value: BigInt(value),
