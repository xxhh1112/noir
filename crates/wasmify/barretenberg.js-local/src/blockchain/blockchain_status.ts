...
type Jsonify<T> = {
  ...
  [P in keyof T]: T[P] extends EthAddress | bigint | Buffer
    ? string
    : T[P] extends Object
    ? Jsonify<T[P]>
    : T[P];
...
export const blockchainAssetToJson = ({
  address,
  ...asset
}: BlockchainAsset): BlockchainAssetJson => ({
  ...asset,
  address: address.toString(),
});

export const blockchainAssetFromJson = ({
  address,
  ...asset
}: BlockchainAssetJson): BlockchainAsset => ({
  ...asset,
...
export const blockchainBridgeToJson = ({
  address,
  ...bridge
}: BlockchainBridge): BlockchainBridgeJson => ({
  ...bridge,
  address: address.toString(),
});

export const blockchainBridgeFromJson = ({
  address,
  ...bridge
}: BlockchainBridgeJson): BlockchainBridge => ({
  ...bridge,
  address: EthAddress.fromString(address),
});
...
export function blockchainStatusToJson(
  ...
  return {
    ...
    defiInteractionHashes: status.defiInteractionHashes.map((v) =>
      v.toString("hex")
    ),
...
export function blockchainStatusFromJson(
  json: BlockchainStatusJson
): BlockchainStatus {
  return {
    ...json,
    rollupContractAddress: EthAddress.fromString(json.rollupContractAddress),
    verifierContractAddress: EthAddress.fromString(
      json.verifierContractAddress
    ),
    ...
    defiInteractionHashes: json.defiInteractionHashes.map((f) =>
      Buffer.from(f, "hex")
    ),
