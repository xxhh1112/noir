...
describe("compute_nullifier", () => {
  ...
  const privateKey = Buffer.from(
    "0b9b3adee6b3d81b28a0886b2a8415c7da31291a5e96bb7a56639e177d301beb",
    "hex"
  );
  ...
  const dummyNullifier = Buffer.from(
  ...
  it("should compute correct nullifier", () => {
    ...
    expect(nullifier1.toString("hex")).toEqual(
      "1d6bac88f87297f2b81d0131534f1eec5f15404bb85721020cccc6497677c9f5"
    );
  });

  it("should commit to claim note and compute its nullifier", () => {
    const bridgeId = BridgeId.fromBigInt(BigInt(456));
    const claimNoteTxData = new ClaimNoteTxData(
      BigInt(100),
      bridgeId,
      noteSecret,
      dummyNullifier
    ...
    expect(nullifier.toString("hex")).toEqual(
      "039395785283f875f10902a7548d83ad959b5a06c8c32943a7735ee2c9f14e1e"
    );
  });

  it("should create correct commitment for defi interaction note", () => {
    const bridgeId = BridgeId.fromBigInt(BigInt(456));
    const note = new DefiInteractionNote(
      bridgeId,
      1,
      BigInt(123),
      BigInt(456),
    ...
    expect(commitment.toString("hex")).toEqual(
      "0196130e904cada31725bd8b7bb73de20eda978c92a2e05cd735429df1c88a47"
    );
  });
  ...
  it("should compute correct alias hash nullifier", () => {
    ...
    expect(nullifier.toString("hex")).toEqual(
      "0c61620f2cef41c6c9401025a658170a6b756d3f5d3af33c8d53f39b21d84ca6"
    );
  });
  ...
  it("should compute correct public key nullifier", () => {
    ...
    expect(nullifier.toString("hex")).toEqual(
      "293e4583639708553c09d48eb546ea2a784c75e5619f099b41fa7ea42b68bde8"
    );
  });
});
