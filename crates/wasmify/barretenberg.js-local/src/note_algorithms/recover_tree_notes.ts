...
export const recoverTreeNotes = (
  ...
  const ownerPubKey = new GrumpkinAddress(
    grumpkin.mul(Grumpkin.one, privateKey)
  );
  return decryptedNotes.map((decrypted, i) => {
    ...
    if (commitment.equals(noteCommitment)) {
      debug(
        `index ${i}: tree commitment ${noteCommitment.toString(
          "hex"
        )} matches note version 1.`
      );
      return note;
    }
    debug(
      `index ${i}: tree commitment ${noteCommitment.toString(
        "hex"
      )} != encrypted note commitment ${commitment.toString("hex")}.`
    );
  });
};
