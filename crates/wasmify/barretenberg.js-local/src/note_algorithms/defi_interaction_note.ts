...
export class DefiInteractionNote {
  static EMPTY = new DefiInteractionNote(
    BridgeId.ZERO,
    0,
    BigInt(0),
    BigInt(0),
...
export const computeInteractionHashes = (
  ...
  return hash.map((h) =>
    toBufferBE(
      BigInt("0x" + h.toString("hex")) % DefiInteractionNote.groupModulus,
      32
    )
  );
};

export const packInteractionNotes = (
  notes: DefiInteractionNote[],
  padTo = notes.length
) => {
  ...
  return toBufferBE(
    BigInt("0x" + hash.toString("hex")) % DefiInteractionNote.groupModulus,
    32
  );
};
