...
export class MerkleTree {
  public static ZERO_ELEMENT = Buffer.from(
  ...
  public async updateElement(index: number, value: Buffer) {
    return await this.updateLeafHash(
      index,
      value.equals(Buffer.alloc(32, 0)) ? this.initialLeafValue : value
    );
  }

  public async updateLeafHash(index: number, leafHash: Buffer) {
    const batch = this.db.batch();
    this.root = await this.updateElementInternal(
      this.root,
      leafHash,
      index,
      this.depth,
  ...
  public async updateLeafHashes(index: number, leafHashes: Buffer[]) {
    while (leafHashes.length) {
      ...
      this.root = await this.updateElementsInternal(
        this.root,
        hashes,
        index,
        this.depth,
