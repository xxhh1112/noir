...
export class SinglePedersen implements Pedersen {
  ...
  constructor(
    ...
    private worker: BarretenbergWorker = wasm as any
  ) {}
  ...
  public async hashToTree(values: Buffer[]) {
    ...
    const resultPtr = await this.worker.call(
      "pedersen__hash_to_tree",
      inputPtr
    );
    const resultNumFields = Buffer.from(
      await this.worker.sliceMemory(resultPtr, resultPtr + 4)
    ).readUInt32BE(0);
    const resultData = Buffer.from(
      await this.worker.sliceMemory(
        resultPtr,
        resultPtr + 4 + resultNumFields * 32
      )
    );
