  ...
  it("does not allow asset id to have value and not in use", () => {
...

    {
      const idVal = replaceBitConfig(
        ...
        new BitConfig(false, true)
      );
      expect(() => BridgeId.fromBigInt(idVal)).toThrow(
        "Inconsistent second input."
      );
    }
    {
      const idVal = replaceBitConfig(
        new BridgeId(0, 1, 2, 0, 4),
        new BitConfig(false, false)
      );
      expect(() => BridgeId.fromBigInt(idVal)).toThrow(
        "Inconsistent second output"
      );
    }
  });
});
