...
describe("validate bridge id", () => {
  it("does not allow identical input assets", () => {
    ...
    expect(() =>
      validateBridgeId(new BridgeId(0, realInputAssetId, 2, realInputAssetId))
    ).toThrow("Identical input assets.");
    expect(() =>
      validateBridgeId(
        new BridgeId(0, virtualInputAssetId, 2, virtualInputAssetId)
      )
    ).toThrow("Identical input assets.");
    expect(() =>
      validateBridgeId(
        new BridgeId(0, realInputAssetId, 2, virtualInputAssetId)
      )
    ).not.toThrow();
  });
  ...
  it("does not allow identical real output assets", () => {
    ...
    expect(() =>
      validateBridgeId(
        new BridgeId(0, 1, realOutputAssetId, 3, realOutputAssetId)
      )
    ).toThrow("Identical output assets.");
    expect(() =>
      validateBridgeId(
        new BridgeId(0, 1, virtualOutputAssetId, 3, virtualOutputAssetId)
      )
    ).not.toThrow();
    expect(() =>
      validateBridgeId(
        new BridgeId(0, 1, realOutputAssetId, 3, virtualOutputAssetId)
      )
    ).not.toThrow();
  });
});
