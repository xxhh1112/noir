...
export class Block {
  ...
  static deserialize(buf: Buffer, offset = 0) {
    ...
    const interactionResult = des.deserializeArray(
      DefiInteractionEvent.deserialize
    );
  ...
  toBuffer() {
    return Buffer.concat([
      ...
      serializeBufferArrayToVector(
        this.offchainTxData.map((b) => serializeBufferToVector(b))
      ),
      serializeBufferArrayToVector(
        this.interactionResult.map((b) => b.toBuffer())
      ),
