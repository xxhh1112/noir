...
export class AccountTx {
  ...
  static fromBuffer(buf: Buffer) {
    ...
    const accountPublicKey = new GrumpkinAddress(
      buf.slice(dataStart, dataStart + 64)
    );
    dataStart += 64;
    const newAccountPublicKey = new GrumpkinAddress(
      buf.slice(dataStart, dataStart + 64)
    );
    dataStart += 64;
    const newSpendingPublicKey1 = new GrumpkinAddress(
      buf.slice(dataStart, dataStart + 64)
    );
    dataStart += 64;
    const newSpendingPublicKey2 = new GrumpkinAddress(
      buf.slice(dataStart, dataStart + 64)
    );
    dataStart += 64;
    ...
    dataStart += adv;
    const spendingPublicKey = new GrumpkinAddress(
      buf.slice(dataStart, dataStart + 64)
    );
