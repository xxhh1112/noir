...
export class Prover {
  ...
  public async createProof(proverPtr: number) {
    ...
    try {
      const circuitSize = await this.proverCall(
        "prover_get_circuit_size",
        proverPtr
      );
      ...
      await this.processProverQueue(proverPtr, circuitSize);
      timer.mark("done");
      const proofSize = await this.proverCall(
        "prover_export_proof",
        proverPtr,
        0
      );
      const proofPtr = Buffer.from(
        await this.wasm.sliceMemory(0, 4)
      ).readUInt32LE(0);
      return Buffer.from(
        await this.wasm.sliceMemory(proofPtr, proofPtr + proofSize)
      );
    } finally {
  ...
  private async processProverQueue(proverPtr: number, circuitSize: number) {
    ...
    debug(
      `starting jobs scalars:${scalarJobs} ffts:${fftJobs} iffts:${ifftJobs}`
    );

    for (let i = 0; i < scalarJobs; ++i) {
      const scalarsPtr = await this.proverCall(
        "prover_get_scalar_multiplication_data",
        proverPtr,
        i
      );
      const scalars = await this.wasm.sliceMemory(
        scalarsPtr,
        scalarsPtr + circuitSize * 32
      );
      const result = await this.pippenger.pippengerUnsafe(
        scalars,
        0,
        circuitSize
      );
      await this.wasm.transferToHeap(result, 0);
      await this.proverCall(
    ...
    for (let i = 0; i < fftJobs; ++i) {
      const coeffsPtr = await this.proverCall(
        "prover_get_fft_data",
        proverPtr,
        0,
        i
      );
      const coefficients = await this.wasm.sliceMemory(
        coeffsPtr,
        coeffsPtr + circuitSize * 32
      );
