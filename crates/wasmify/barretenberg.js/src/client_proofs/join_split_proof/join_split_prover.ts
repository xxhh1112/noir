...
export class JoinSplitProver {
  ...
  public async loadKey(keyBuf: Buffer) {
    ...
    await worker.transferToHeap(
      Transfer(keyBuf, [keyBuf.buffer]) as any,
      keyPtr
    );
  ...
  public async getKey() {
    ...
    try {
      const keySize = await worker.call(
        "join_split__get_new_proving_key_data",
        0
      );
      const keyPtr = Buffer.from(await worker.sliceMemory(0, 4)).readUInt32LE(
        0
      );
      const buf = Buffer.from(
        await worker.sliceMemory(keyPtr, keyPtr + keySize)
      );
      await worker.call("bbfree", keyPtr);
      return buf;
    } finally {
