...
export class JoinSplitVerifier {
  ...
  public async computeKey(pippenger: SinglePippenger, g2Data: Uint8Array) {
    ...
    await this.worker.call(
      "join_split__init_verification_key",
      pippenger.getPointer(),
      0
    );
  }

  public async getKey() {
    const keySize = await this.worker.call(
      "join_split__get_new_verification_key_data",
      0
    );
    const keyPtr = Buffer.from(
      await this.worker.sliceMemory(0, 4)
    ).readUInt32LE(0);
    const buf = Buffer.from(
      await this.worker.sliceMemory(keyPtr, keyPtr + keySize)
    );
  ...
  public async loadKey(
    ...
    await this.worker.call(
      "join_split__init_verification_key_from_buffer",
      keyPtr,
      0
    );
    await this.worker.call("bbfree", keyPtr);
  ...
  public async verifyProof(proof: Buffer) {
    ...
    const verified = (await this.worker.call(
      "join_split__verify_proof",
      proofPtr,
      proof.length
    ))
      ? true
