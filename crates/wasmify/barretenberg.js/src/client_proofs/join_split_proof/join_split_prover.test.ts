...
describe("join_split_proof", () => {
  ...
  const createAndCheckProof = async (joinSplitProver: JoinSplitProver) => {
    ...
    const expectedNullifier1 = noteAlgos.valueNoteNullifier(
      inputNote1Enc,
      privateKey
    );
    const expectedNullifier2 = noteAlgos.valueNoteNullifier(
      inputNote2Enc,
      privateKey
    );
  ...
  describe("join_split_mock_proof_generation", () => {
    ...
    beforeAll(async () => {
      ...
      debug(
        `vk hash: ${sha256
          .hash(await joinSplitVerifier.getKey())
          .toString("hex")}`
      );
    });
