...
export class SingleNoteDecryptor implements NoteDecryptor {
  ...
  public async batchDecryptNotes(keysBuf: Buffer, privateKey: Buffer) {
    ...
    const mem = await this.worker.call(
      "bbmalloc",
      keysBuf.length + privateKey.length
    );
    ...
    await this.worker.call(
      "notes__batch_decrypt_notes",
      mem,
      mem + keysBuf.length,
      numKeys,
      mem
    );
    const dataBuf: Buffer = Buffer.from(
      await this.worker.sliceMemory(mem, mem + numKeys * decryptedNoteLength)
    );
