...
export class TreeClaimNote {
  static EMPTY = new TreeClaimNote(
    BigInt(0),
    BridgeId.ZERO,
    0,
    BigInt(0),
  ...
  static deserialize(buf: Buffer, offset: number) {
    return {
      elem: TreeClaimNote.fromBuffer(
        buf.slice(offset, offset + TreeClaimNote.LENGTH)
      ),
  ...
  static fromBuffer(buf: Buffer) {
    ...
    let offset = 32;
    const bridgeId = BridgeId.fromBuffer(
      buf.slice(offset, offset + BridgeId.ENCODED_LENGTH_IN_BYTES)
    );
    offset += 32;
