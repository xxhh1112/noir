...
describe.skip("world_state_db", () => {
  ...
  beforeEach(async () => {
    worldStateDb = new WorldStateDb(
      `/tmp/world_state_db_${randomBytes(32).toString("hex")}.db`
    );
  ...
  it("should be initialized with correct metadata", () => {
    expect(worldStateDb.getRoot(0)).toEqual(
      WorldStateConstants.EMPTY_DATA_ROOT
    );
    expect(worldStateDb.getRoot(1)).toEqual(
      WorldStateConstants.EMPTY_NULL_ROOT
    );
    expect(worldStateDb.getRoot(2)).toEqual(
      WorldStateConstants.EMPTY_ROOT_ROOT
    );
    expect(worldStateDb.getRoot(3)).toEqual(
      WorldStateConstants.EMPTY_DEFI_ROOT
    );
  ...
  it("should get correct hash path", async () => {
    ...
    const expectedLast =
      "02a12922daa0fe8d05620d98096220a86d9ebf4d9552dc0fbd3862b9c48f7ab9";
  ...
  it("should read and write standard I/O sequentially", async () => {
    ...
    const buffers = await Promise.all(
      values.map((_, i) => worldStateDb.get(0, BigInt(i)))
    );
    for (let i = 0; i < num; ++i) {
      expect(buffers[i]).toEqual(values[i]);
    }

    const hashPaths = await Promise.all(
      values.map((_, i) => worldStateDb.getHashPath(0, BigInt(i)))
    );
    for (let i = 0; i < num; ++i) {
