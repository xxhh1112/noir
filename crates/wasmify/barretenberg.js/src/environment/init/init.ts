...
export class InitHelpers {
  ...
  public static getInitRoots(chainId: number): Roots {
    const { initDataRoot, initNullRoot, initRootsRoot } =
      getInitData(chainId).initRoots;
    return {
  ...
  public static async writeAccountTreeData(
    ...
    accountData.forEach((account) => {
      if (account.notes.note1.length !== NOTE_LENGTH) {
        throw new Error(
          `Note1 has length ${account.notes.note1.length}, it should be ${NOTE_LENGTH}`
        );
      }
      if (account.notes.note2.length !== NOTE_LENGTH) {
        throw new Error(
          `Note2 has length ${account.notes.note2.length}, it should be ${NOTE_LENGTH}`
        );
      }
      if (account.alias.aliasHash.length !== ALIAS_HASH_LENGTH) {
        throw new Error(
          `Alias hash has length ${account.alias.aliasHash.length}, it should be ${ALIAS_HASH_LENGTH}`
        );
      }
  ...
  public static parseAccountTreeData(data: Buffer) {
    ...
    for (let i = 0; i < numAccounts; i++) {
      ...
      const signingKeys: SigningKeys = {
        ...
        signingKey2: data.slice(
          start + SIGNING_KEY_LENGTH,
          start + 2 * SIGNING_KEY_LENGTH
        ),
      };
  ...
  public static async populateDataAndRootsTrees(
    ...
    if (rollupSize) {
      ...
      if (notesRequired > entries.length) {
        await merkleTree.put(
          dataTreeIndex,
          BigInt(notesRequired - 1),
          Buffer.alloc(32, 0)
        );
      }
    }
  ...
  public static async populateNullifierTree(
    accounts: AccountData[],
    merkleTree: WorldStateDb,
    nullTreeIndex: number
  ) {
    ...
    console.log(
      `Batch inserting ${entries.length} notes into nullifier tree...`
    );
    await merkleTree.batchPut(entries);
