...
describe("RollupProofData", () => {
  ...
  it("should throw if the number of totalTxFees is wrong", () => {
    ...
    const totalTxFees = [...Array(RollupProofData.NUMBER_OF_ASSETS)].map(() =>
      BigInt(0)
    );
  ...
  it("should have correct deposit sum", () => {
    ...
    const expected = inners
      .map((i) => new RollupDepositProofData(i))
      .reduce((a, i) => a + i.publicValue, BigInt(0));
  ...
  it("should get tx ids from buffer", () => {
    const inners = [randomDepositProofData(), randomDepositProofData()];
    const rollupProofData = createRollupProofData(inners);
    const expected = rollupProofData.innerProofData
      .filter((i) => !i.isPadding())
      .map((i) => i.txId);
    const txIds = RollupProofData.getTxIdsFromBuffer(
