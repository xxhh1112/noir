...
export class WorldState {
  ...
  public async init(subTreeDepth: number) {
    ...
    debug(
      `initialising data tree with depth ${treeSize} and zero element of ${subTreeRoot.toString(
        "hex"
      )}`
    );
    try {
      this.tree = await MerkleTree.fromName(
        this.db,
        this.pedersen,
        "data",
        subTreeRoot
      ...
      this.tree = await MerkleTree.new(
        this.db,
        this.pedersen,
        "data",
        treeSize,
  ...
  public async buildFullHashPath(
    noteIndex: number,
    immutableHashPath: HashPath
  ) {
    ...
    const fullHashPath = new HashPath(
      immutableHashPath.data.concat(mutablePath.data)
    );
    return fullHashPath;
  }
