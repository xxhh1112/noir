        ...
        match value {
            ...
            TomlTypes::ArrayNum(arr_num) => {
                let array_elements: Vec<_> =
                    arr_num.into_iter().map(|elem_num| parse_str(&elem_num.to_string()).unwrap().unwrap()).collect();
                check_toml_map_duplicates(
                    &mut field_map,
                    parameter,
            ...
            TomlTypes::ArrayString(arr_str) => {
                let array_elements: Vec<_> =
                    arr_str.into_iter().map(|elem_str| parse_str(&elem_str).unwrap().unwrap()).collect();
                check_toml_map_duplicates(
                    &mut field_map,
                    parameter,
                    InputValue::Vec(array_elements),
